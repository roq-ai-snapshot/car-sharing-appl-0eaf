...
import { useRoqClient, useDriverFindManyWithCount } from 'lib/roq';
...
export function DriverListPage(props: DriverListPageProps) {
      ...
      convertQueryToPrismaUtil(
        {
          ...
          searchTermKeys: ['license_number.contains'],
          ...params.filters,
        },
        'driver',
      ),
  ...
  const { data, error, isLoading, mutate } = useDriverFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.driver.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    { id: 'license_number', header: 'License Number', accessorKey: 'license_number' },
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.license_expiry_date ? format(record?.license_expiry_date, 'dd-MM-yyyy') : '',
    },
