...
import { useRoqClient, useDriverFindFirst } from 'lib/roq';
...
function DriverEditPage() {
  ...
  const { data, error, isLoading, mutate } = useDriverFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: DriverInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.driver.update({
        data: values as RoqTypes.driver,
        where: {
          id,
        },
      });
